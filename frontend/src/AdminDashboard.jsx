...
import { 
  ...
  TrendingUp, DollarSign, Trash2, X, Clock, MapPin, 
  CreditCard, User, ChevronRight, Search 
} from "lucide-react";
    ...
    <div className="flex min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      <aside className="fixed top-0 left-0 h-full w-64 bg-slate-900 text-white flex flex-col shadow-2xl z-30 transition-transform duration-300 transform translate-x-0">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <SidebarItem icon={<ShoppingBag size={20} />} label="Pesanan Masuk" active={activeTab === "orders"} count={orders.filter(o => o.status === 'Pending').length} onClick={() => setActiveTab("orders")} />
      ...
      {/* MAIN CONTENT (Margin Left menyesuaikan Sidebar) */}
      <main className="flex-1 ml-64 p-8 lg:p-12">
        <header className="flex justify-between items-end mb-10">
          <div>
            <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">
                {activeTab === 'dashboard' ? "Dashboard Overview" : activeTab === 'orders' ? "Manajemen Pesanan" : "Inventaris Produk"}
            </h2>
                    ...
                    orders.map((order) => (
                        <div key={order.id} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 group">
                            ...
                            <div className="flex flex-wrap gap-3 pt-2">
                                <ActionButton label="Proses" onClick={() => handleStatusChange(order.id, "Diproses")} color="bg-yellow-100 text-yellow-700 hover:bg-yellow-200" icon="🛠" />
                                <ActionButton label="Kirim" onClick={() => handleStatusChange(order.id, "Dikirim")} color="bg-blue-100 text-blue-700 hover:bg-blue-200" icon="🚚" />
                                <ActionButton label="Selesai" onClick={() => handleStatusChange(order.id, "Selesai")} color="bg-green-100 text-green-700 hover:bg-green-200" icon="✅" />
                                <div className="ml-auto">
                                    <ActionButton label="Batalkan" onClick={() => handleStatusChange(order.id, "Batal")} color="bg-red-50 text-red-600 hover:bg-red-100" icon="❌" />
                                </div>
                            </div>
                        </div>
                    ))
                 ...
                 <div className="lg:col-span-2 space-y-4">
                     {products.length === 0 ? (
                        ...
                        <div key={p.id} className="bg-white p-4 rounded-2xl border border-gray-200 flex items-center justify-between group hover:shadow-lg transition-all duration-300">
                            ...
                            <button onClick={() => handleDeleteProduct(p.id)} className="p-3 text-gray-300 hover:text-red-600 hover:bg-red-50 rounded-xl transition">
                                <Trash2 size={20} />
                            </button>
                        </div>
                    ))}
